...
namespace PlazmaGames.Core.Utils
{
    ...
    public static class UniversalRenderPipelineUtils
    {
        ...
        public static List<T> GetRendererFeature<T>(UniversalRenderPipelineAsset asset = null) where T : ScriptableRendererFeature
        {
        ...
        {
            var features = GetRendererFeature<T>(asset);
            if (features == null)
                return;

            foreach (var feature in features) feature.SetActive(active);
        }
